...
import { FixtureError, loadSpec, pruneSpec, runCli, writeTextFile } from '@fixture-automation/openapi-fixtures';

import { OPENAPI_TYPES_HELP, OPENAPI_TYPES_USAGE } from './common/openapi-types-cli.const.ts';
...
const run = async (): Promise<void> => {
  const { positionals, values } = parseArgs({ options: cliOptions, allowPositionals: true });
  ...
  const [specUrl, second, third] = positionals;

  if (specUrl === undefined || positionals.length > 3) throw new FixtureError(OPENAPI_TYPES_USAGE);

  if (second !== undefined && third !== undefined) return generatePruned(specUrl, second, third);

  await generate(specUrl, second);
};

await runCli(TOOL, run);
