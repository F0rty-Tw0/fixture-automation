...
/** Spawns `src/cli.ts` with the workspace source condition and resolves for failures too. */
export const spawnFixturesCli = async (args: string[], cwd: string): Promise<CliResult> => {
  ...
  const spawn = (settle: (result: CliResult) => void): void => {
    ...
    execFile(process.execPath, commandArgs, { cwd, env, timeout: CLI_TIMEOUT }, done);
  };

  return new Promise(spawn);
};

/** `true` when the output carries a Node stack frame, which no CLI failure may print. */
export const hasStackFrame = (text: string): boolean => {
  return text.split('\n').some((line: string): boolean => line.startsWith('    at '));
};
